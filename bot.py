...
BOT_TOKEN = "6924904511:AAHdqTcvCH1vGWJxfSeofSAs0K5PALDsaw"
...
async def start(message: types.Message):
    ...
    if len(ref) > 1 and ref[1] != user_id:
        ...
        if ref[1] not in referrals.get(user_id, []):
            ...
            await bot.send_message(ref[1], f"✅ Новый реферал! +500 голды")
    ...
    text += f"👥 Рефералов: {count} / {MIN_REFERRALS}\n"
    text += f"📌 Для вывода нужно {MIN_REFERRALS} рефералов\n\n"
